...
/**
*  UnionDenverProductionHawaApp for Union Denver
*  Smartthings App
...
*/
definition(
  name: "Mysore test only application",
  namespace: "Qwinix",
  author: "ksatish@qwinix.io",
  ...
  mappings {
      path("/api/v1/community/devices/get_hub"){
        action: [
        GET: "locationHubDetails"
        ]
    }
    path("/api/v1/community/devices"){
         action: [
      GET: "index"
      ]
    }
    ...
    path ("/community/devices/:qhublabel/:device_id") {
      action: [
      POST: "commandDeviceById"
      ]
    }
    path("/model/:cmd") {
      action: [
  ...
  // Custom Methods - DO NOT DELETE THIS - KEDARNAG MUKANAHALIIPATNA, VENKATESH
  def locationHubDetails(){
      def hub = location.hubs[0]
    log.debug "Location hub List ============= ${hubs}"
   return [hub: hub.id, name: hub.name, zigbeeId: hub.zigbeeId]
  }
  ...
  def switchResponse(device, qhublabel) {
      log.debug "Device ${device}"
    log.debug "lable ${qhublabel}"
      return [name: device.getName(), status: device.currentValue('switch'), level: device.currentValue("level"), id: device.id, qhublabel: qhublabel, userlabel: device.label]
  }

  def identifySwitchQhublabel(switchableDevice) {
    def qhublabel = 'switch';
    log.debug "Switchable device ${switchableDevice}"
  ...
  def updateLock(l, command) {
  log.debug "inside updateLock +++++++++++++ ${command}"
    switch(command) {
      case "lock":
        l.lock()
         return [name: l.getName(), lock: 'locked', id: l.id, qhublabel: 'lock', userlabel: l.label]
      break
      case "unlock":
        l.unlock()
         return [name: l.getName(), lock: 'unlocked', id: l.id, qhublabel: 'lock', userlabel: l.label]
      break
      default:
  ...
  def updateLevel(l, command) {
    switch(command) {
      case "on":
      l.setLevel(99)
        return [name: l.getName(), status: l.currentValue('switch'), level: 99, id: l.id, qhublabel: state.devicemap[l.id], userlabel: l.label]
      break
      case "off":
      l.setLevel(0)
       return [name: l.getName(), status: l.currentValue('switch'), level: 0, id: l.id, qhublabel: state.devicemap[l.id], userlabel: l.label]
      break
      case 25:
      l.setLevel(25)
       return [name: l.getName(), status: l.currentValue('switch'), level: 25, id: l.id, qhublabel: state.devicemap[l.id], userlabel: l.label]
      break
      case 50:
      l.setLevel(50)
       return [name: l.getName(), status: l.currentValue('switch'), level: 50, id: l.id, qhublabel: state.devicemap[l.id], userlabel: l.label]
      break
      case 75:
      l.setLevel(75)
       return [name: l.getName(), status: l.currentValue('switch'), level: 75, id: l.id, qhublabel: state.devicemap[l.id], userlabel: l.label]
      break
      case 99:
      l.setLevel(99)
      return [name: l.getName(), status: l.currentValue('switch'), level: 99, id: l.id, qhublabel: state.devicemap[l.id], userlabel: l.label]
  ...
  def commandDeviceById() {
    ...
    def deviceid = request.JSON?.device_id
    def dtype = state.devicemap[deviceid]
    def cmdname = getCmdName(request.JSON?.device, dtype)
    if (dtype == null) {
      httpError(404, "No device installed with id " + deviceid + " in the qwinix smart app. Check the samsung smarthings developer dashboard.")
    }
  ...
  def getCmdName(device, dtype){
  log.debug "Inside get device +++++++++++++ ${device}"
    log.debug "Inside get dtype+++++++++++++ ${dtype}"
  def command = ""
  switch(dtype){
  case "light":
  case "switch":
  ...
  case "lock":
     ...
     break
     default:
       httpError(404, "$dtype is not valid device type")
